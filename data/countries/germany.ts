...
function duration(edge: Edge, start: number, end: number): number {
  const d =
    Math.min(end, edge.arrival.time) - Math.max(start, edge.departure.time);
  return Math.max(0, d);
}
...
function basic(edge: Edge, day: Day): number {
  if (weekEnd(edge, day)) {
    return (
      duration(edge, h(6, 0), h(9, 0)) + duration(edge, h(20, 0), h(23, 0))
    );
  }
...
function metro(edge: Edge, day: Day): number {
  if (weekEnd(edge, day)) {
    return (
      duration(edge, 9 * 60, 20 * 60) +
      duration(edge, (9 + 24) * 60, (20 + 24) * 60)
    );
  }

  return (
    duration(edge, 6 * 60, 20 * 60) +
...
function rules(
  ...
  return [
    Rule.perKm(
      (2.63 * nightDuration) / totalDuration,
      `Prix „Nacht“ 2,63€/km sur ${nightShare}%`,
      RuleCategory.Tracks
    ),
    Rule.perKm(
      (4.76 * basicDuration) / totalDuration,
      `Prix „Basic“  4,76 €/km sur ${basicShare}%`,
      RuleCategory.Tracks
    ),
    Rule.perKm(
      (metroMid * metroDuration) / totalDuration,
      `Prix „Metro“  ${metroMid.toFixed(
        2
      )} €/km, vitesse moyenne ${avgSpeed.toFixed(0)} km/h sur ${metroShare}%`,
      RuleCategory.Tracks
    ),
