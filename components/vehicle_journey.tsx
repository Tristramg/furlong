...
import { VehicleJourney as VJ, TrainEdge } from '../lib/types';
...
const VehicleJourney: React.FunctionComponent<Props> =({ vj }) => {

  const first: TrainEdge = _.head(vj.edges);
  const tail: TrainEdge[] = _.drop(vj.edges);
  const last: TrainEdge = _.last(tail)
  const middle: TrainEdge[] = _.dropRight(tail);
  ...
  <div>
    <Segment edge={first} position='first'></Segment>
    {middle.map((edge, index) => <Segment key={index} edge={edge} position='middle'></Segment>)}
    <Segment edge={last} position='last'></Segment>
  </div>
  <div className="flex gap font-bold border border-gray-400 rounded m-1 px-1">
